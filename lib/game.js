;(function (global) {
  ...
  var Game = Election.Game = function (options) {
    ...
    this.musicEl = options.musicEl;
    this.billImg = options.billImg;
    this.deanImg = options.deanImg;
    this.newspaperImg = options.paperImg;
  ...
  Game.prototype.collideWithHand = function(object) {
    if(object instanceof Election.Bill) {
      this.audioEl.src = "audio/cha_ching_register.mp3";
      this.score += 1;
    } else if(object instanceof Election.Newspaper) {
      this.audioEl.src = "audio/dean_scream.mp3";
      this.pressPoints -= 1;
  ...
  Game.prototype.run = function() {
    ...
    this.musicEl.src = 'audio/politician.mp3';
    this.musicEl.play();
    this.runDirections();
  };
    ...
    this.timer1 = setInterval(
      function() {
        ...
        if(game.isOver()){
          ...
          game.musicEl.pause();
          if(game.isWon()) {
            game.audioEl.src = "audio/stars_and_stripes.mp3";
            game.finishView.renderWin();
          } else {
            game.audioEl.src = "audio/sad_trombone.mp3";
            if(game.isMoneyLoss()) {
