;(function (global) {
  ...
  Game.prototype.instantiateFallingObjects = function() {
    for(var i = 0; i < 50; i++) {
      this.bills[i] = new Election.Bill({ img: this.billImg });
      // this.papers[i] = new Election.Newspaper({ img: this.newspaperImg });
    }
    this.allObjects = [this.hand].concat(this.bills).concat(this.papers);
    this.fallingObjects = this.bills.concat(this.papers);
  };
